...
- (void)modCellDidTapToggle:(UITableViewCell *)cell {
    ...
    if (!ok) {
        UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Lỗi" message:err.localizedDescription preferredStyle:UIAlertControllerStyleAlert];
...
- (void)modCellDidTapDelete:(UITableViewCell *)cell {
    ...
    if (!ip) return;
    ModItem *m = self.filteredMods[ip.row];
    UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Xác nhận xóa" message:m.displayName preferredStyle:UIAlertControllerStyleAlert];
    [ac addAction:[UIAlertAction actionWithTitle:@"Hủy" style:UIAlertActionStyleCancel handler:nil]];
    [ac addAction:[UIAlertAction actionWithTitle:@"Xóa" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        NSError *err = nil;
        if ([[ModService sharedService] deleteMod:m error:&err]) {
            ...
            UIAlertController *errAc = [UIAlertController alertControllerWithTitle:@"Xóa thất bại" message:err.localizedDescription preferredStyle:UIAlertControllerStyleAlert];
    ...
    if (u) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [[UIApplication sharedApplication] openURL:u options:@{} completionHandler:^(BOOL success) {
                if (!success) {
                    UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Không thể mở liên kết" message:urlStr preferredStyle:UIAlertControllerStyleAlert];
                    [ac addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:nil]];
                    [self presentViewController:ac animated:YES completion:nil];
                }
            }];
